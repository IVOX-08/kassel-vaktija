...
fun DhikrScreen(modifier: Modifier = Modifier) {
    ...
    val all = DhikrData.ITEMS
    // Rotating order: the remembrances you've already seen cycle to the bottom, so across repeated
    // visits you move through the whole list and then start over — indefinitely. The pointer that
    // marks "where you are" is persisted across opens.
    val prefs = remember { context.getSharedPreferences(PREFS, Context.MODE_PRIVATE) }
    val startIndex = remember { ((prefs.getInt(KEY_ROTATION, 0) % all.size) + all.size) % all.size }
    val items = remember(startIndex) { List(all.size) { all[(startIndex + it) % all.size] } }

    val listState = rememberLazyListState()
    // How many items the user has actually seen this visit (the furthest card reached).
    val seenCount = remember { mutableIntStateOf(1) }
    LaunchedEffect(listState) {
        snapshotFlow { listState.layoutInfo.visibleItemsInfo.lastOrNull()?.index ?: 0 }
            .collect { last -> if (last + 1 > seenCount.intValue) seenCount.intValue = last + 1 }
    }
    // On leaving, move the seen ones to the bottom (advance the pointer) for next time.
    DisposableEffect(Unit) {
        onDispose {
            val next = (startIndex + seenCount.intValue) % all.size
    ...
    LazyColumn(
        ...
        items(items, key = { it.transliteration }) { dhikr -> DhikrCard(dhikr, lang) }
    }
}

private const val PREFS = "dhikr_prefs"
